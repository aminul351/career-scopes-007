...
const CatDetails = () => {
  ...
  return (
    <div className="p-6 px-12">
      <p className="mt-8 text-4xl text-center font-bold text-gray-700 font-textStyle">{category.title}</p>
      ...
      <h2 className="text-3xl font-semibold my-12">Here are some <span className="font-bold font-textStyle">Sub Categories</span> of {category.title}</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
        {category.sub_cat.map((sub) => (
          <Link key={sub.id} to={`/categories/${category.id}/${sub.id}`}>
            <div className="p-4  border rounded-lg shadow hover:shadow-md transition-transform transform hover:-translate-y-1 duration-700  cursor-pointer">
              <img className="h-[250px] w-full" src={sub.image} alt="" />
              <h3 className="my-2 font-bold text-3xl  font-textStyle">{sub.name}</h3>
